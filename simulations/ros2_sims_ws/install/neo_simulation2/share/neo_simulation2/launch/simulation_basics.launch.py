...
def generate_launch_description():
    ...
    moving_srv = Node(package='services_pkg',executable='service',output='screen')
    stop_srv = Node(package='services_pkg',executable='service2',output='screen')

    move_robot_as_pkg_prefix = get_package_share_directory('move_robot_as')
    move_robot_as_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource([move_robot_as_pkg_prefix, '/launch/action_server.launch.py']),
